...
describe('registerController Comprehensive Unit Tests', () => {
  ...
  describe('Format Validation Logic (Mocked Helpers)', () => {
    test('should return 400 if validateName returns false', async () => {
      // Arrange
  ...
  describe('Additional Normalization & Edge Cases', () => {
    test('should use normalized (lowercase) email when querying for existing user', async () => {
      // Arrange
      req.body.email = '  UPPER@EXAMPLE.COM  ';
      userModel.findOne.mockResolvedValue(null);
      hashPassword.mockResolvedValue('hashed');
      ...
      // Assert - findOne must be called with the lowercase trimmed email
      expect(userModel.findOne).toHaveBeenCalledWith({
        email: 'upper@example.com',
      });
    });

    test('should include address field in userModel constructor call', async () => {
      // Arrange
      req.body.address = '456 Another Street';
      userModel.findOne.mockResolvedValue(null);
      hashPassword.mockResolvedValue('hashed');
      setupMockUser(req.body);
      ...
      // Assert
      expect(userModel).toHaveBeenCalledWith(
        expect.objectContaining({ address: '456 Another Street' }),
      );
    });

    test('should pass whitespace-only password to hashPassword without trimming', async () => {
      // Arrange - password is intentionally not trimmed by the controller
      req.body.password = '   validlooking   ';
      userModel.findOne.mockResolvedValue(null);
      hashPassword.mockResolvedValue('hashed');
      setupMockUser(req.body);
      ...
      // Assert - password is passed as-is (untrimmed) to hashPassword
      expect(hashPassword).toHaveBeenCalledWith('   validlooking   ');
    });

    test('should return 201 with success false absent from existing user response', async () => {
      // Arrange
      userModel.findOne.mockResolvedValue({ email: 'test@example.com' });
      ...
      // Assert - existing-user response has success: false
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.send).toHaveBeenCalledWith(
